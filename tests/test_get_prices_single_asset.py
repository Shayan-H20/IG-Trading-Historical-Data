...
class TestGetHistoricalPrices:
    ...
    def test_get_prices_single_asset_num_points(self, mocker):
        ...
        assert all(prices.round(2).values[0] == mock_hist_data.mock_prices_num_points.round(2).values[0])
        assert all(prices.round(2).values[1] == mock_hist_data.mock_prices_num_points.round(2).values[1])
    ...
    def test_get_prices_single_asset_dates_with_time_interval(self, mocker):
        ...
        """ test .get_prices_single_asset() using 'dates' with time interval """
        ...
        assert all(prices.columns == mock_hist_data.mock_prices_dates_time_interval.columns)
        assert all(prices.index == mock_hist_data.mock_prices_dates_time_interval.index)
        assert all(prices.round(2).values[0] == mock_hist_data.mock_prices_dates_time_interval.round(2).values[0])
        assert all(prices.round(2).values[1] == mock_hist_data.mock_prices_dates_time_interval.round(2).values[1])
